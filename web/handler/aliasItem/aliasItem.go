...
func GetData() (string, error) {
	// 尝试从缓存中获取数据
	//TODO: 可能需要上写锁
	cachedData, err := service.RDB.Get(handler.Ctx, "alias_items").Result()
	if errors.Is(err, redis.Nil) {
		// 缓存未命中，从数据库中获取数据
		data, err := service.AliasItemClient.FetchData(service.ListItemClient)
		if err != nil {
			return "", err
		}
		cache := string(utils.Marshal(*data))
		// 缓存数据
		service.RDB.Set(handler.Ctx, "alias_items", cache, 0)
...
func HandleInsertNode(c *gin.Context) {
	var item models.AliasItem
	if err := c.ShouldBindJSON(&item); err != nil {
		c.JSON(http.StatusBadRequest, handler.StatusBad("参数错误", nil))
		return
	}
	if err := service.AliasItemClient.InsertNode(&item); err != nil {
		c.JSON(500, handler.StatusBad("出错了……", nil))
		return
	}

	// 清除缓存
	service.RDB.Del(handler.Ctx, "alias_items")
	data, _ := GetData()
...
//	}
//
//	if err := service.AliasItemClient.UpdateNode(&item); err != nil {
//		c.JSON(500, handler.StatusBad("出错了……", nil))
//		return
//	}
//
...
func HandleDeleteNode(c *gin.Context) {
	...
	if err := service.AliasItemClient.DeleteNodeByID(uint(elemID)); err != nil {
		c.JSON(500, handler.StatusBad("出错了……", nil))
		return
	}
